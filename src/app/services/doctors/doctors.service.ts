...
export class DentistsService {
  ...
  createDentist(dentist:Dentists): Observable<APIResponse<Dentists>>{
    return this.http.post<APIResponse<Dentists>>(this.API_URL, dentist).pipe(catchError(this._handleHttpErrors(new Dentists())));
  }

  updateService(dentist:Dentists): Observable<APIResponse<Dentists>>{
    return this.http.put<APIResponse<Dentists>>(`${this.API_URL}/update/${dentist._id}`, dentist).pipe(catchError(this._handleHttpErrors(new Dentists())));
  }
